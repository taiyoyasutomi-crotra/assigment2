// デモ用シードデータを投入する。
// 「運営者1名、イベント2件、申込8件+3件」の状態を作る。
// 会員はアカウント不要(アンケート方式の申込)のため、members には作らない。
...
try {
  ...
  await client.query(
    `insert into members (display_name, email, role) values
     ('運営 太郎', 'admin@example.com', 'admin')`
  );

  // 申込者10名(名前・ニックネーム・メール)。
  ...
  const people = [
    ["佐藤 花子", "はなちゃん", "hanako.sato@example.com"],
    ["鈴木 一郎", "いっちー", "ichiro.suzuki@example.com"],
    ["高橋 美咲", "みさき", "misaki.takahashi@example.com"],
    ["田中 健太", "けんけん", "kenta.tanaka@example.com"],
    ["伊藤 さくら", "さくらん", "sakura.ito@example.com"],
    ["渡辺 大輔", "だいちゃん", "daisuke.watanabe@example.com"],
    ["山本 結衣", "ゆいゆい", "yui.yamamoto@example.com"],
    ["中村 翔太", "しょうた", "shota.nakamura@example.com"],
    ["小林 愛", "あいぼん", "ai.kobayashi@example.com"],
    ["加藤 拓海", "たくみん", "takumi.kato@example.com"],
  ].map(([name, nickname, email], i) => [
    name,
    nickname,
  ...
  // イベント1: 定員5に対して申込8件入り → 選定で先着5名が当選、3名が待機。
  ...
  for (let i = 0; i < 8; i++) {
    const [name, nickname, email] = people[i];
    await client.query(
      `insert into applications (event_id, applicant_name, nickname, email, token, applied_at)
       values ($1, $2, $3, $4, $5, now() - interval '1 hour' * $6)`,
      [event1, name, nickname, email, token(), 8 - i]
    );
  }
  ...
  for (let i = 0; i < 3; i++) {
    ...
    await client.query(
      `insert into applications (event_id, applicant_name, nickname, email, token)
       values ($1, $2, $3, $4, $5)`,
      [event2, name, nickname, email, token()]
    );
  }
  ...
  console.log("  イベント: ファンミーティング Vol.5(定員5・申込8件 → 先着5名当選・3名待機)");
